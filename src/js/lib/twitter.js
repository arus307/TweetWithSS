(function() {
  ...
  var Twitter = {
    ...
    isLoggedIn: function(cb) {
       ...
       chrome.storage.local.get(['oauth_token', 'oauth_token_secret'],function(result){
        Twitter.oauth_token = result.oauth_token;
        Twitter.oauth_token_secret = result.oauth_token_secret;
      });
    ...
    setUserData:function(){
      ...
      if(Twitter.oauth_token == false || Twitter.oauth_token_secret == false){
        return;
      }

      Twitter.api('account/verify_credentials','GET',{},function(response){
        Twitter.user_name = response.name;
        Twitter.user_screen_name = response.screen_name;
        Twitter.user_profile_image_url = response.profile_image_url;

        chrome.storage.local.set({ 'user_name': response.name, 'user_screen_name': response.screen_name,'user_profile_image_url':response.profile_image_url });
    ...
    api: function(path /* params obj, callback fn */) {
      ...
      var accessor = {consumerSecret: consumer_secret, tokenSecret: Twitter.oauth_token_secret},
        message = {
          action: API_URL + path,
          method: method,
          parameters: [['oauth_consumer_key', consumer_key], ['oauth_signature_method', 'HMAC-SHA1'],['oauth_token',Twitter.oauth_token]]
        };
    ...
    uploadApi: function(path /* params obj, callback fn */) {
      var args = Array.prototype.slice.call(arguments, 1),
          fn = false,
          params = {},
          method = 'GET';
      ...
      $[method.toLowerCase()](UPLOAD_API_URL + path, p.join('&'), fn).error(function(res) {
        if(res && res.responseText && res.responseText.match(/89/)) {
          Twitter.authenticate();
        }
      });
